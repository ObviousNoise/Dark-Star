...
render_debug_info 		= true--true


perfomance = 
{
	roll_compensation_limits	= {math.rad(-180.0), math.rad(180.0)}, --limitations while rolling the aircraft.
	pitch_compensation_limits	= {math.rad(-45.0), math.rad(45.0)},   --limitations while pithing the aircraft.

	
	tracking_azimuth   			= { -math.rad(30),math.rad(30)}, --Degrees in left and right.
	tracking_elevation 			= { -math.rad(30),math.rad(30)}, --degrees in up and down. 
	
	scan_volume_azimuth 	= math.rad(60),		--is left+right so 60 is -30,30+
	scan_volume_elevation	= math.rad(30),		--is up+down    so 30 is -15,15+
	scan_beam				= math.rad(60),     --is Right thru left Scanning Angle of 15 degres till full sweep.
	scan_speed				= math.rad(3*60),   --how fast can it scan.
	
	
	max_available_distance  = 20000.0,--200*60000.0, --Maximum range.
	dead_zone 				= 100.0,  --How close can it detect before being useless.
	
	ground_clutter = -- ground clutter depending on surface.
	{   --spot RCS = A + B * random + C * random 
		sea		   	   = {0 ,0,0},  --Sea/Ocean
		land 	   	   = {0 ,0,0},	--Ground surface,Mountains.	
		artificial 	   = {0 ,0,0},  --horizon.
		rays_density   = 0.01,      --densety of the clutter.
		max_distance   = 60000,     --how far it can see clutter.
	}
...
function post_initialize()
	...
	print_message_to_user("Radar Suite Online")
...
function SetCommand(command,value)
	print_message_to_user(string.format("Radar on",command,value))
